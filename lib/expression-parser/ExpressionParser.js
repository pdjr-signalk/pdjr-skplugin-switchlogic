...
module.exports = class ExpressionParser {
    ...
    parsePrefixExpression(expression) {
        console.log("ExpressionParser.parsePrefixExpression(%s)...", expression);
        var tokens = null;
        var stack = [];
        ...
        if (expression) {
            ...
            if ((tokens) && (tokens.length > 0)) {
                tokens.reverse().forEach(token => {
                    console.log(">> processing token %s...", token);
                    if (this.precedence(token) ==  0) {
                        console.log(">>>> token is operand, parsing and pushing result to stack");
                        stack.push(this.parsers.operand.parser(token));
                    } else {
                        switch (this.parsers[token].arity) {
                            case 1:
                                if (stack.length > 0) {
                                    console.log(">>>> token is unary operator, replacing stack top with parsed function");
                                    stack.push(this.parsers[token].parser(stack.pop()));
                                }
                                break;
                            case 2:
                                if (stack.length > 1) {
                                    console.log(">>>> token is binary operator, replacing stack top two items with parsed function");
